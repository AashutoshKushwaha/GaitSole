...
def load_real(path):
    """STUB for real data. Implement an adapter that returns a list of raw-trial
    dicts (keys = config.RAW_COLS + 'time', values = 1-D float arrays at FPS).
    See README for the Schreiber (OpenSim IK) / Camargo recipes."""
    raise NotImplementedError(
        "Hook your real dataset loader here -- return a list of raw-trial dicts "
        "in the same format as data.make_synthetic_trial(). See README.md.")
